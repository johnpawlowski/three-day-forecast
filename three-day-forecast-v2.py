...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fetch 3-day forecast for location"
    )
    parser.add_argument("city_name", type=str, help="City name to search.")
    parser.add_argument("--units", type=str, default="metric", help="Units (imperial/metric)")
    return parser.parse_args()

# get the location coordindates via the Geocoding API
def get_coord(query, units):
    try:
        response = requests.get(COORD_URL, params={"q": query, "appid": api_key, "units": units})
    ...
    if data == []:
        ...
        return coord, name, units

# get the 5-day / 3-hour forecast via the Forecast 5-Day/3-Hour API
def get_weather(lat, lon, units):
    try:
        response = requests.get(WEATHER_URL, params={"lat": lat, "lon": lon, "appid": api_key, "units": units})
        response.raise_for_status()
    except requests.exceptions.HTTPError as e:
        print(f"API error: {e}")
        return None
...
coord_result = get_coord(query=args.city_name, units=args.units)
if coord_result is None:
    print("Could not retrieve coordinates. Exiting.")
    exit()
coordinates, resolved_name, units = coord_result

# use coordinates to query for raw weather data
weather = get_weather(coordinates[0], coordinates[1], units)
if weather is None:
